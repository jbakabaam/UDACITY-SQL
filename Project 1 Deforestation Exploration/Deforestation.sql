...
CREATE VIEW forestation AS
    SELECT f.country_code code, f.country_name country,
        ...
        100.0*(f.forest_area_sqkm / (l.total_area_sq_mi * 2.59))
        AS percentage
...
SELECT  
    (curr.forest_area_sqkm - prev.forest_area_sqkm) / 
    prev.forest_area_sqkm * 100 AS percentage
FROM forest_area AS curr
JOIN forest_area AS prev
ON  (curr.year = '2016' AND prev.year = '1990'
    AND curr.country_name = 'World'
...
SELECT ROUND(CAST((region_forest_1990/region_area_1990)*100 AS NUMERIC),2) 
    ...
    region  
FROM (
    SELECT SUM(a.forest_area_sqkm) region_forest_1990,
